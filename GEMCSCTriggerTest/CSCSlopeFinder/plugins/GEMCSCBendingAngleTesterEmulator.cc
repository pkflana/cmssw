...
GEMCSCBendingAngleTesterEmulator::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup){
  ...
  for (CSCCorrelatedLCTDigiCollection::DigiRangeIterator j = primaryLCTs->begin(); j != primaryLCTs->end(); j++){
    ...
    for (; CSCCorrLCT != last; ++CSCCorrLCT){
      ...
      data_.BendingAngleRealistic = data_.has_LCT_match1 ? data_.LCT_BendingAngle_GE1 : data_.LCT_BendingAngle_GE2;//START HERE for lct matching
        ...
        for (unsigned int track_id = 0; track_id < emtf_tracks->size(); ++track_id) {
          ...
          if ((track.Mode_CSC() & 8) == 8){
            ...
            for (unsigned int hit_id = 0; hit_id < emtf_hits.size(); ++hit_id) {
              ...
              has_me11 = true;
              n_me11_lcts++;
              if ((debug) or (true && data_.LCT_CSC_ME1a)){
              ...
              if ((hit.Chamber() == LCTDetId.chamber()) && (hit.Endcap() == LCTDetId.zendcap())){
                ...
                int csccorrlctstrip = CSCCorrLCT->getStrip();
                if (hit.Ring()==4){
                  csccorrlctstrip -= 128;
                }
                if ((hit.Strip() == csccorrlctstrip) and (hit.Wire() == CSCCorrLCT->getKeyWG())){
