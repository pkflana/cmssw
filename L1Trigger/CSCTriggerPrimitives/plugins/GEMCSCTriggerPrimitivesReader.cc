...
private:
  ...
  std::vector<int> t_sloperesolution;
  std::vector<int> t_gemlayer;
  std::vector<int> t_slopeex;
  std::vector<int> t_bx;
        ...
        for (int cham = 1; cham <= 36; cham++) {
          ...
          for (auto digiIt = range.first; digiIt != range.second; digiIt++) {
            if ((*digiIt).isValid()) {
              ...
              t_bend.push_back((*digiIt).getBend());
              t_slope.push_back((*digiIt).getSlope());
              uint16_t sloperesolution = (*digiIt).getRawSlopeResolution();
              t_sloperesolution.push_back(sloperesolution);
              t_slopeex.push_back((*digiIt).getSlopeEx(6));
              t_gemlayer.push_back((*digiIt).getGemLayerUsedForSlopeComputation());
              t_bx.push_back((*digiIt).getBX());
              t_pattern.push_back((*digiIt).getPattern());
              t_run3pattern.push_back((*digiIt).getRun3Pattern());
              bool isme1a = false;
...
TTree* GEMCSCTriggerPrimitivesReader::bookTTree_LCT(){
  ...
  t->Branch("bend", "std::vector<int>", &t_bend);
  t->Branch("slope", "std::vector<int>", &t_slope);
  t->Branch("sloperesolution", "std::vector<int>", &t_sloperesolution);
  t->Branch("slopeex", "std::vector<int>", &t_slopeex);
  t->Branch("gemlayer","std::vector<int>", &t_gemlayer);
  t->Branch("bx", "std::vector<int>", &t_bx);
  t->Branch("pattern", "std::vector<int>", &t_pattern);
  t->Branch("run3pattern", "std::vector<int>", &t_run3pattern);
